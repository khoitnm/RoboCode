...
public class AvoidOneAreaTooLongMoveHelper {
    ...
    private static Point2D findDestinationOutsideArea(Rectangle2D battleField, Rectangle2D tooLongMoveArea, Collection<Enemy> enemies) {
        Rectangle2D[][] battleFieldQuarters = BattleFieldUtils.splitToParts(battleField, 2, 2);
        List<Rectangle2D> battleFieldParts = ListUtils.toList(battleFieldQuarters);
        List<Rectangle2D> newParts = excludeTooLongMoveArea(battleFieldParts, tooLongMoveArea);
        List<Rectangle2D> leastRiskyParts = findLeastRiskyParts(newParts);
        Point2D destination = findBestDestinationInParts(leastRiskyParts);
        return destination;
    }
}
