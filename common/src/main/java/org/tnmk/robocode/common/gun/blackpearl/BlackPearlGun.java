...
public class BlackPearlGun implements InitiableRun, OnScannedRobotControl {
    ...
    public void runInit() {
		...
		field = new Rectangle2D.Double(18, 18, robot.getBattleFieldWidth() - 36, robot.getBattleFieldHeight() - 36);
...
//		} while(true);
	...
	public void onScannedRobot(ScannedRobotEvent e) {
		...
		double latVel = (eVelocity) * Math.sin(eHeading = e.getHeadingRadians() - (eAbsBearing = Utils.normalRelativeAngle(robot.getHeadingRadians() + (e.getBearingRadians()))));
		eX = robot.getX() + Math.sin(eAbsBearing) * (eDistance = e.getDistance());
		eY = robot.getY() + Math.cos(eAbsBearing) * eDistance;
...
//		setTurnRadarRightRadians(Math.sin(eAbsBearing - getRadarHeadingRadians()));
		...
		if (firePower > 0) {
			...
			w.dist+=w.bulletVel;
			w.shotOrigin = new Point2D.Double(robot.getX(), robot.getY());
	...
	Object readObject(String fileName) {
		try {
			ObjectInputStream ois = new ObjectInputStream(new GZIPInputStream(new FileInputStream(robot.getDataFile(fileName))));
	...
	private void goTo(Point2D destination) {
		ticksSinceReverse++;
		double angle = Utils.normalRelativeAngle(Math.atan2((destination.getX() - robot.getX()), (destination.getY()- robot.getY())) - robot.getHeadingRadians());
		double turnAngle = Math.atan(Math.tan(angle));
        robot.setTurnRightRadians(turnAngle);
        robot.setAhead(destination.distance(robot.getX(), robot.getY()) * (angle == turnAngle ? 1 : -1) * Math.cos(turnAngle));
	}
